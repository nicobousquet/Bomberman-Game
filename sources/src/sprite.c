...
static void bomb_load() {
    bomb_img[EXPLOSION - 1] = image_load(BOMB_EXPLOSION);
    bomb_img[TTL1 - 1] = image_load(BOMB_TTL1);
    bomb_img[TTL2 - 1] = image_load(BOMB_TTL2);
    bomb_img[TTL3 - 1] = image_load(BOMB_TTL3);
    bomb_img[TTL4 - 1] = image_load(BOMB_TTL4);
}
...
SDL_Surface *sprite_get_bomb(int ttl) {
    assert(bomb_img[ttl - 1]);
    return bomb_img[ttl - 1];
}
